...
Der folgende Versuch behandelt das Thema GSM. GSM ist die Abkürzung für Global System for Mobile Communications und ein Standard für die volldigitalen Mobilfunknetze. Wir haben ihn bereits kennen gelernt, da alle unsere Handys darauf beruhen. GSM ermöglicht die eigentliche Telefonie, eine Datenübertragung und das Versenden und Empfangen von SMS, Short Message Services. Mittlerweile wurden für die Datenübertragung leistungsfähigere Standards, wie UMTS und LTE entwickelt, jedoch ist GSM noch nicht wegzudenken. 
...
Mit dem Tool lsursp wird überprüft, ob das USPR \ref{uspr} vom System erkannt wird.

Nachdem festgestellt wurde, dass das USRP angeschlossen und vom System erkannt wurde, kann der eigentliche Versuch beginnen. Hierzu wird das Tool kal mit dem Kommando \textit{ kal -s -DCS} aufgerufen. Kal führt einen Umgebungsscan durch, das bedeutet, dass alle Frequenzen, die im DCS 1800 Band liegen und empfangen werden können, aufgelistet werden können.
...
In der Tabelle \ref{fig:Frequenzbereich} werden die Zusammenhänge von Up- und Downlink klar. Die Startfrequenz ist ein fest definierter Wert von 1710,2 Mhz. Die zu GSM 1800 gehörenden Channels sind von 512 bis 885 definiert, daher auch der feste Offset von 512.
...
\subsubsection{Versuchsdurchführung}
Vor Beginn des Versuchs muss der Umgebungsscan aus \ref{frequenzenAuflisten} wiederholt werden. Dadurch kann sicher gestellt werden, dass die Messung mit einer aktiven, bzw. gerade gesendeten Frequenz durchgeführt wird. Nachdem eine Frequenz gefunden wurde, beginnt der eigentliche Versuch mit dem Kommando  \textit{dbusrp-f 699219}. Dieses Kommando startet ein Analysetool, das sowohl den Frequenz- als auch den Amplitudenbereich ausgibt. Die Zahl \glqq 699219\grqq gibt die Frequenz un Herz an, auf der gemessen werden soll.
...
Es soll ein Anruf auf die Telefonnummer 2600, die dem echo-Dienst entspricht, durchgeführt werden. Dazu benötigen wir den am Anfang beschriebenen Versusaufbau sowie ein GSM-Fähiges Mobiltelefon das in dem Netz registriert ist. 
...
Mitgeschnitten werden die Daten die über das \(U_m\) Interface gesendet werden. Das \(U_m\) Interface ist eine Funkschnittstelle die für die Übertragung zwischen der Mobile-Station und dem Base Transceiver Station zuständig ist. Das \(U_m\) Interface arbeitet auf den untersten drei Layern des ISO-OSI-Referenzmodells.
...
Im Header werden Informationen zu der verwendeten Version, der Länge des Headers, des Types der übertragenden Daten, sowie die ARFCN und nähere Informationen zur Signalstärke angegeben.
...
Unser Hauptaugenmerk legen wir auf die Pakete die als SIP gekennzeichnet sind. Hier kann man am besten nachverfolgen was gerade für eine Aktion durchgeführt wird. Bevor eine Interaktion mit dem Echo-Server stattfinden kann, muss eine Verbindung aufgebaut werden. Der Verbindungsaufbau beginnt mit einer \textit{Invite} Nachricht.

Bevor das jedoch passiert wird vorher auf dem Layer 2 eine Setup Nachricht gesendet. Wie man in Abbildung \ref{fig:Lapdm Setup Nachricht} sehen kann wird hier ebenfalls die Bearer Capability angefordert. Die Bearer Capability bedeutet, dass ein Gerät, in unserem Fall die MS, eine bestimmte Transportleistung vom Netz anfordert. 
...
\begin{figure}[H]
  ...
  \caption{Bestätigungs Nachricht im SIP Protokoll}
...
Wird der Versuch mit einem eigenen Mobiltelefon ausgeführt, muss sich dieses erst am Netz anmelden. Diese Anmeldung wird vom Netz mit einer SMS bestätigt.
...
\begin{itemize}
	...
	\item Transport Protocol Data Unit (TPDU) - Übertragung der Nutzerdaten
\end{itemize}

Wie man sehr gut in der Abbildung \ref{fig:sms_send_receive} sehen kann, ist der gesendete Text unten in der GSM SMS TPDU ebenfalls ein Teil der ankommenden Nachricht. Dieser beinhaltet, neben dem gesendeten Text, auch die Zeit in der die SMS gesendet wurde, sowie die IMSI Adresse und Telefonnummer. Die Inhalte können von dem von uns gemachten Versuch abweichen, da die Daten von einem älteren Versuch stammen. 
